...
interface RecordingsTabProps {
  ...
  newRecording: { title: string; description: string; gdriveLink: string; recordingDate: string; thumbnailUrl: string; duration: string };
  setNewRecording: (recording: { title: string; description: string; gdriveLink: string; recordingDate: string; thumbnailUrl: string; duration: string }) => void;
  addRecording: (e: React.FormEvent) => void;
...
export default function RecordingsTab({
  ...
  const filteredRecordings = recordings.filter(recording => 
    ...
    (recording.description && recording.description.toLowerCase().includes(recordingSearch.toLowerCase()))
  );
      ...
      <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
        ...
        <form onSubmit={addRecording} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input
            ...
            onChange={(e) => setNewRecording(prev => ({ ...prev, recordingDate: e.target.value }))}
            required
            className="rounded-lg bg-gray-700 text-gray-100 px-3 py-2 border border-gray-600 focus:border-green-500 focus:outline-none"
          />
          <input
            ...
            onChange={(e) => setNewRecording(prev => ({ ...prev, gdriveLink: e.target.value }))}
            required
            className="rounded-lg bg-gray-700 text-gray-100 px-3 py-2 border border-gray-600 focus:border-green-500 focus:outline-none"
          />
          <input
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <div className="text-sm text-gray-400 mb-2">
                    <span>📅 {new Date(recording.recordingDate).toLocaleDateString()}</span>
                    {recording.duration && (
