...
export async function POST(req: Request) {
  try {
    ...
    if (action === "start") {
      ...
      if (existingStatus) {
        return NextResponse.json({ error: "Voting is already active" }, { status: 400 });
      }

      // Create new voting status with end time (2 days from now)
      const votingEndTime = new Date(now.getTime() + (2 * 24 * 60 * 60 * 1000)); // 2 days
      ...
      if (!activeStatus) {
        return NextResponse.json({ error: "No active voting found" }, { status: 400 });
      }
