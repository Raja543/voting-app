...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async signIn({ user, account, profile }) {
      ...
      try {
        // Early exit for credentials provider or missing account object
        if (!account || account.provider === "credentials") {
          return true;
        }

        // Safely retrieve the email from supported OAuth profiles
        const email =
          user.email ||
          (profile as any)?.data?.email ||
          (profile as any)?.email;

        // Reject sign-in if email is missing (especially Twitter OAuth)
        if (!email) {
